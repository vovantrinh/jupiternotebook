"""
Script để chuẩn bị dataset cân bằng cho training
Chia test set thành train/val với cả good và defect images
"""
...
def prepare_balanced_dataset(
    ...
    output_dir='datasets/mvtec_balanced/bottle',
    train_ratio=0.7,
    seed=42
):
    """
    Chuẩn bị dataset cân bằng từ MVTec dataset
    ...
    # 1. Lấy good images từ train và test
    ...
    for img_path in tqdm(train_good_images):
        ...
        os.chmod(dst_path, 0o644)  # Đặt quyền đọc/ghi
    ...
    print(f"Found {len(defect_images)} defect images from {len(defect_categories)} categories: {defect_categories}")
    ...
    print("="*60)
    print(f"TRAIN:")
    print(f"  Good:   {len(train_good_images)} images")
    print(f"  Defect: {len(train_defect_images)} images")
    print(f"  Total:  {len(train_good_images) + len(train_defect_images)} images")
    print(f"\nVALIDATION:")
    print(f"  Good:   {len(val_good_images)} images")
    print(f"  Defect: {len(val_defect_images)} images")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='datasets/mvtec_balanced/bottle',
